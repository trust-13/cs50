...
void record_preferences(int ranks[])
{
    for (int j = 0; j < candidate_count; j++)
    {
        for (int k = j+1; k < candidate_count; k++)// why dosent "for(int k = j++; ...)" work?
        {
...
void sort_pairs(void)
{
    ...
    for (int i = 0; i < pair_count -1; i++)
    {
        for (int c = 0; c < pair_count -1; c++)
        {
            ...
            {
                savewin = pairs[c].winner;
                savelose = pairs[c].loser;
                pairs[c].winner = pairs[c+1].winner;
                pairs[c].loser = pairs[c+1].loser;
                pairs[c+1].winner = savewin;
                pairs[c+1].loser = savelose;
            }
        }
    }
    return;
